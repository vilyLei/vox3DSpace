...
GLuint loadTextures()
{
    ...
    unsigned char* data    = stbi_load(imgUrl.c_str(), &width, &height, &nrChannels, rgbaEnabled ? STBI_rgb_alpha : STBI_rgb);
    GLuint         texture = GL_ZERO;
    if (data)
    {
        std::boolalpha(std::cout);
        std::cout << "successed to load rgbaEnabled: " << rgbaEnabled << std::endl;
        std::cout << "successed to load a texture." << std::endl;
        std::cout << ">>>> tex width:" << width << ", height: " << height << ", nrChannels: " << nrChannels << std::endl;
