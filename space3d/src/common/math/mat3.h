...
namespace baseMath {
	...
	template <
		...
		constexpr TMat33<T>::TMat33(A m00, B m01, C m02,
			D m10, E m11, F m12,
			G m20, H m21, I m22) : m_value{col_type(m00, m01, m02),col_type(m10, m11, m12),col_type(m20, m21, m22), }
	{
	...
	constexpr typename TMat33<U>::col_type  operator *(const TMat33<T>& lhs, const TVec3<U>& rhs) {
	...
	template <typename T, typename U>
	constexpr typename TMat33<U>::row_type  operator *(const TVec3<U>& lhs, const TMat33<T>& rhs) {
	...
	template<typename T, typename U>
	constexpr typename std::enable_if<std::is_arithmetic<U>::value, TMat33<T>>::type 
	 ...
	 */
	template<typename T>
	constexpr typename TMat33<T>::col_type  diag(const TMat33<T>& m) {
	...
	typedef TMat33<double> mat3;
	typedef TMat33<float> mat3f;
...
namespace std {
	template <typename T>
	constexpr void swap(baseMath::TMat33<T>& lhs, baseMath::TMat33<T>& rhs) noexcept
	{
