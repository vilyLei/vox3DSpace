...
VEC3 texNormalValue;
	...
	float strength = g_material.normalMapStrength;//#TODO 法线图调整
	if (g_isFront) {
...
void main()
{
	...
	float matReflectionIntensity = 0.75f; //反射强度
	float matFrontColorScale = 1.0f; //正面颜色缩放
	float matSideColorScale = 1.0f;//侧面颜色缩放
	float matMetalness = 0.0f; //金属度
	g_material.roughness = 0.35f; //光滑度
	float sideLightIntensity = 0.0f;//侧面光强度
	g_material.reflection = matReflectionIntensity;
#elif defined DEBUG_MATTE
	int matIsSpecularPass = 0;//
	float matReflectionIntensity = 0.15f; //反射强度
	float matFrontColorScale = 1.0f; //正面颜色缩放
	float matSideColorScale = 1.0f;//侧面颜色缩放
	float matMetalness = 0.0f; //金属度
	g_material.roughness = 0.5f; //光滑度
	float sideLightIntensity = 0.0f;//侧面光强度
	g_material.reflection = matReflectionIntensity;
#elif defined DEBUG_SHINY
	int matIsSpecularPass = 0;//
	float matReflectionIntensity = 0.4f; //反射强度
	float matFrontColorScale = 1.0f; //正面颜色缩放
	float matSideColorScale = 1.0f;//侧面颜色缩放
	float matMetalness = 0.0f; //金属度
	g_material.roughness = 0.3f; //光滑度
	float sideLightIntensity = 0.0f;//侧面光强度
	g_material.reflection = matReflectionIntensity;
#else
	int matIsSpecularPass = g_material.isSpecularPass;//
	float matReflectionIntensity = g_material.reflection;//反射强度
	float matFrontColorScale = g_material.frontScale;//正面颜色缩放
	float matSideColorScale = g_material.sideScale;//侧面颜色缩放
	float matMetalness = GetMetalness();//金属度//g_material.metalness;
	...
	//环境图计算
	float envIntensity = g_Scene.envLightIntensity; //环境图亮度
	...
	//阴影深度偏移计算。法线和光线角度大,偏移越大；范围在[0.0002, 0.001]
	...
	if (dotNLShadow > 0.001) //阴影在 阴影光线和法线方向相反的时候不起作用
	{
...
#pragma region PBRLightComputing
	...
	for (int iLight = 0; iLight < g_LightArray.lightNum; iLight++)
	{
		...
		VEC4 lightDir = light.direction; //将世界坐标系的方向转到视点坐标系
...
#pragma endregion PBRLightComputing
	...
	specularTotal *= GammaCorrectionInv(GetSpecularColor()); //高光颜色叠加
