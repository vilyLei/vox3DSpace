...
private:
    ...
    void init() {
        ...
        createInstance();
        window_ = std::make_unique<Window>(600, 700, "Projectile + Bounce");
        ...
        swapChain_ = std::make_unique<VulkanSwapChain>(device_.get(), surface_, 600, 700);
        ...
        if (std::filesystem::exists("Release/scripts/test_vis_gravity"))
            scriptDir = "Release/scripts/test_vis_gravity/";
        else if (std::filesystem::exists("Debug/scripts/test_vis_gravity"))
            scriptDir = "Debug/scripts/test_vis_gravity/";
        else
            scriptDir = "scripts/test_vis_gravity/";
        ...
        createSyncObjects();
        std::cout << "Init OK: 3 objects created." << std::endl;
    ...
    void createInstance() {
        vk::ApplicationInfo ai{}; ai.pApplicationName="test_vis_gravity"; ai.apiVersion=VK_API_VERSION_1_0;
...
int main() {
    std::cout << "=== TEST: test_vis_gravity ===" << std::endl;
    std::cout << "Purpose: Verify parabolic y=floor+v0*t-0.5*g*t^2, mod period" << std::endl;
    try { Test_test_vis_gravity test; test.run(); return 0; }
