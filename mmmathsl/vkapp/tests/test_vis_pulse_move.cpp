...
class Test_test_vis_pulse_move {
public:
...
private:
    ...
    void init() {
        ...
        createInstance();
        window_ = std::make_unique<Window>(800, 600, "Pulse+Move Coupled");
        ...
        if (std::filesystem::exists("Release/scripts/test_vis_pulse_move"))
            scriptDir = "Release/scripts/test_vis_pulse_move/";
        else if (std::filesystem::exists("Debug/scripts/test_vis_pulse_move"))
            scriptDir = "Debug/scripts/test_vis_pulse_move/";
        else
            scriptDir = "scripts/test_vis_pulse_move/";
        ...
        createSyncObjects();
        std::cout << "Init OK: 4 objects created." << std::endl;
    ...
    void createInstance() {
        vk::ApplicationInfo ai{}; ai.pApplicationName="test_vis_pulse_move"; ai.apiVersion=VK_API_VERSION_1_0;
...
int main() {
    std::cout << "=== TEST: test_vis_pulse_move ===" << std::endl;
    std::cout << "Purpose: Verify 4-channel script coupling: color,rot,pos,scale" << std::endl;
    try { Test_test_vis_pulse_move test; test.run(); return 0; }
