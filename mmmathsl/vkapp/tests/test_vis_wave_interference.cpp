...
class Test_test_vis_wave_interference {
public:
...
private:
    ...
    void init() {
        ...
        createInstance();
        window_ = std::make_unique<Window>(800, 600, "Wave Interference");
        ...
        swapChain_ = std::make_unique<VulkanSwapChain>(device_.get(), surface_, 800, 600);
        ...
        if (std::filesystem::exists("Release/scripts/test_vis_wave_interference"))
            scriptDir = "Release/scripts/test_vis_wave_interference/";
        else if (std::filesystem::exists("Debug/scripts/test_vis_wave_interference"))
            scriptDir = "Debug/scripts/test_vis_wave_interference/";
        else
            scriptDir = "scripts/test_vis_wave_interference/";
        ...
        createSyncObjects();
        std::cout << "Init OK: 5 objects created." << std::endl;
    ...
    void createInstance() {
        vk::ApplicationInfo ai{}; ai.pApplicationName="test_vis_wave_interference"; ai.apiVersion=VK_API_VERSION_1_0;
...
int main() {
    std::cout << "=== TEST: test_vis_wave_interference ===" << std::endl;
    std::cout << "Purpose: Verify two-wave superposition with color encoding" << std::endl;
    try { Test_test_vis_wave_interference test; test.run(); return 0; }
