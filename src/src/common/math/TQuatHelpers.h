...
namespace baseMath {
	...
	public:
		...
		template<typename RT>
		friend inline
			constexpr TVec3<T>  operator *(const QUATERNION<T>& q, const TVec3<RT>& v) {
		 ...
		 */
		friend inline
			constexpr QUATERNION<T>  operator *(QUATERNION<T> q, T scalar) {
			// don't pass q by reference because we need a copy anyways
			return q *= scalar;
		}
		friend inline
			constexpr QUATERNION<T>  operator *(T scalar, QUATERNION<T> q) {
			// don't pass q by reference because we need a copy anyways
			return q *= scalar;
		}
	...
	public:
		...
		template<typename RT>
		friend inline
			constexpr T  dot(const QUATERNION<T>& p, const QUATERNION<RT>& q) {
			return p.x * q.x +
				p.y * q.y +
				p.z * q.z +
				p.w * q.w;
		...
		friend inline
			constexpr T  length2(const QUATERNION<T>& q) {
			return dot(q, q);
		}
		...
		friend inline
			constexpr T  real(const QUATERNION<T>& q) {
			return q.w;
		}

		friend inline
			constexpr TVec3<T>  imaginary(const QUATERNION<T>& q) {
			return q.xyz;
		}

		friend inline
			constexpr QUATERNION<T>  unreal(const QUATERNION<T>& q) {
			return QUATERNION<T>(q.xyz, 0);
		}

		friend inline
			constexpr QUATERNION<T>  cross(const QUATERNION<T>& p, const QUATERNION<T>& q) {
			return unreal(p*q);
		}
		...
		friend inline
			QUATERNION<T>  pow(const QUATERNION<T>& q, T a) {
		...
		friend inline
			constexpr QUATERNION<T>  lerp(const QUATERNION<T>& p, const QUATERNION<T>& q, T t) {
			return ((1 - t) * p) + (t * q);
		}

		friend inline
			constexpr QUATERNION<T>  nlerp(const QUATERNION<T>& p, const QUATERNION<T>& q, T t) {
			return normalize(lerp(p, q, t));
		}

		friend inline
			constexpr QUATERNION<T>  positive(const QUATERNION<T>& q) {
			return q.w < 0 ? -q : q;
		}
	};
