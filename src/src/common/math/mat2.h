...
namespace baseMath {
	...
	constexpr typename TMat22<U>::col_type  operator *(const TMat22<T>& lhs, const TVec2<U>& rhs) {
	...
	template <typename T, typename U>
	constexpr typename TMat22<U>::row_type  operator *(const TVec2<U>& lhs, const TMat22<T>& rhs) {
	...
	template<typename T, typename U>
	constexpr typename std::enable_if<std::is_arithmetic<U>::value, TMat22<T>>::type
	 ...
	 */
	template<typename T>
	constexpr typename TMat22<T>::col_type  diag(const TMat22<T>& m) {
	...
	typedef TMat22<double> mat2;
	typedef TMat22<float> mat2f;
}
...
namespace std {
	template <typename T>
	constexpr void swap(baseMath::TMat22<T>& lhs, baseMath::TMat22<T>& rhs) noexcept
	{
