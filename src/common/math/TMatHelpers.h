...
namespace baseMath {
	...
	namespace matrix {
		...
		MATRIX  gaussJordanInverse(const MATRIX& src) {
		...
		template <typename MATRIX>
		MATRIX  fastInverse2(const MATRIX& x) {
			typedef typename MATRIX::value_type T;
		...
		template <typename MATRIX>
		MATRIX  fastInverse3(const MATRIX& x) {
			typedef typename MATRIX::value_type T;
		...
		constexpr MATRIX_R  multiply(const MATRIX_A& lhs, const MATRIX_B& rhs) {
			// pre-requisite:
		...
		template <typename MATRIX>
		typename MATRIX::col_type  diag(const MATRIX& m) {
	...
	public:
		...
		template<typename U>
		friend constexpr BASE<T>  operator *(const BASE<T>& lhs, const BASE<U>& rhs) {
	...
	public:
		 ...
		 */
		friend inline BASE<T>  inverse(const BASE<T>& matrix) {
			return matrix::inverse(matrix);
		}
		friend inline  BASE<T>  transpose(const BASE<T>& m) {
			return matrix::transpose(m);
		}
		friend inline  T  trace(const BASE<T>& m) {
	...
	public:
		...
		friend inline BASE<T>  abs(BASE<T> m) {
			for (size_t col = 0; col < BASE<T>::NUM_COLS; ++col) {
				m[col] = abs(m[col]);
			}
			return m;
	...
	public:
		...
		>
			...
			return eulerZYX(roll, yaw, pitch);
		}
